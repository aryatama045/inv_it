...
<div class="row">
    <div class="col-12 ">
        <div class="card mb-5">
            ...
            <form id="formAction" class="row g-3" action="<?= base_url($mod.'/'.$func.'/check/'.$header['nomor_transaksi']); ?>" method="POST">
                ...
                <table class="table table-striped" >
                        ...
                        <tr >
                            ...
                            <th class="text-bold text-uppercase">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="checkAllCheckbox" onchange="toggleAllCheckboxes(this)">
                                    <label class="form-check-label" for="checkAllCheckbox"> CHECK</label>
                                </div>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="form-check">
                                    <input name="pic_check_checked[<?= $val['no_urut'] ?>]" value="1" <?= ($val['pic_check_checked'])? 'checked':''; ?> data-checked-from-db="<?= ($val['pic_check_checked']) ? 1 : 0 ?>" class="form-check-input checkboxItem" type="checkbox" id="stackedCheck<?= $val['no_urut'] ?>" onchange="preventUncheck(this, <?= $val['pic_check_checked'] ?>)">
...
<script type="text/javascript">
    ...
    /* Fungsi untuk mencegah uncheck checkbox yang sudah di-check dari database */
    function preventUncheck(checkbox, wasCheckedFromDB) {
        if (wasCheckedFromDB && !checkbox.checked) {
            // Jika checkbox ini sebelumnya checked dari DB tapi user coba uncheck
            // Maka force tetap checked
            checkbox.checked = true;
            alert('Checkbox ini tidak bisa di-uncheck karena sudah tersimpan di database');
        }
    }
    /* End preventUncheck */

    /* Fungsi untuk toggle semua checkbox pic_check_checked[] */
    function toggleAllCheckboxes(checkAllCheckbox) {
        var isChecked = checkAllCheckbox.checked;
        // Gunakan selector yang match semua checkbox pic_check_checked dengan pattern name
        var checkboxes = document.querySelectorAll('input[type="checkbox"][name^="pic_check_checked["]');
        checkboxes.forEach(function(checkbox) {
            // Jika checkbox sudah checked dari DB, jangan di-uncheck
            var wasCheckedFromDB = checkbox.getAttribute('data-checked-from-db');
            if (!wasCheckedFromDB || isChecked) {
                checkbox.checked = isChecked;
            }
        });
    }
    /* End Fungsi Toggle All Checkbox */
</script>
