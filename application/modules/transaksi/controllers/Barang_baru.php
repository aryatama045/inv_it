...
class Barang_baru extends Admin_Controller  {
	...
	{
		$this->render_template('barang_baru/index',$this->data);
	}
	...
	public function show($id)
	{
		...
		if($this->data['header']['nomor_transaksi']){
			if($this->data['header']['manual'] == 'True'){
				$this->render_template('barang_baru/detail_manual',$this->data);
			}else{
				$this->render_template('barang_baru/detail',$this->data);
			}
		}else{
	...
	public function tambah()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
			...
			$this->starter();
			$this->render_template('barang_baru/tambah',$this->data);
		}
	...
	public function tambah_manual()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
			...
			if($create_form) {
				...
				$this->session->set_flashdata('error', 'Silahkan Cek kembali data yang di input !!');
				redirect('transaksi/barang_baru/tambah_manual', 'refresh');
			}

		}else{
			$this->starter();
			$this->render_template('barang_baru/tambah_manual',$this->data);
		}
	...
	public function edit($id)
	{
        ...
        if ($this->form_validation->run() == TRUE) {
			...
			if($edit_form) {
				...
				$this->session->set_flashdata('error', 'Silahkan Cek kembali data yang di input !!');
				redirect('master/barang_baru/edit/'.$id, 'refresh');
			}
			...
			if($this->data['barang']['kode_barang']){
				$this->render_template('barang_baru/edit',$this->data);
			}else{
				$this->session->set_flashdata('error', 'Silahkan Cek kembali data yang di input !!');
				redirect('master/barang_baru/edit/'.$id, 'refresh');
			}
	...
	public function print_action()
	{
        ...
        if(isset($output)){
			...
			if($output[0]['header']['manual'] == 'True'){
				$this->load->view('barang_baru/print_manual',$send_data);
			}else{
				$this->load->view('barang_baru/print',$send_data);
			}
